...
export default function Profile() {
    ...
    const handleTeacher = async (event) => {
        ...
        try {
            ...
            body: JSON.stringify({ type: messageOption, message: messageContent }),
            });
            console.log(response.status)
            if (response.status === 200) {
                // If the login is successful, redirect to the website with the desired route.
            ...
            <section className="profile-bg center">
                <div className="list">
                    <ul >
                        ...
                        <li onClick={() => {handleTabClick('teachers')}}>Teachers</li>
                        ...
                        <form action="" onSubmit={handleSubmit} >
                            <div>
                                <div className="input-container">
                                    <input onChange={(event) => {setFirstName(event.target.value)}} placeholder={firstName} type="text"/>
                                    <label className="transition">First name</label>
                                </div>
                                <div className="input-container">
                                    <input onChange={(event) => {setLastName(event.target.value)}} placeholder={lastName} type="text"/>
                                    <label className="transition">Last name</label>
                                </div>
                                <div className="input-container">
                                    <input onChange={(event) => {setPassword(event.target.value)}} placeholder="*********" type="password"/>
                                    <label className="transition">Password</label>
                                </div>
                                <div className="input-container">
                                    <input onChange={(event) => {setconfirmPassword(event.target.value)}} placeholder="*********" type="password"/>
                    ...
                    <div className="teacher-msg">
                        <form className="center" onSubmit={handleTeacher}>
                            <div className=' input-container center'>
                                ...
                                <p className="error-msg"  id='first-name'>Please Your Course</p>
                            </div>
                            <div className='input-container center'>
                                <div className='center teacher-txt'>
                                    <label className="transition">Enter your Message and Zoom Link</label>
                                    <textarea 
                                        rows="4"
                                        cols="50" 
                                        name="comment" 
                    ...
                    {activeTab === "teachers" && teacher &&
                                    ...
                                    <ul>
                                        ...
                                        <li>
                                            ...
                                            <div className="btns">
                                                <button onClick={() => ActionRequestAccept(teacher[0].user._id, teacher[0].type)} className='accept btn center'><img src="/img/icons8-select-checkmark-symbol-to-choose-true-answer-96.png" alt="" /></button>
                                                <button onClick={() => ActionRequestReject(teacher[0].user._id, teacher[0].type)} className='decline btn center'><img src="/img/x-solid.svg" alt="" /></button>
                                            </div>
                                        </li>
                                    </ul>
                                </div>
